...
/** POST /api/zadmin/security/unlock/[id]/deny */
export async function POST(
  ...
  if (req.status !== 'pending') {
    return jsonError(`Cannot deny a request with status: ${req.status}`, 'INVALID_STATUS', 409);
  }

  const now = new Date().toISOString();
  const { data: updated, error: updateErr } = await dbAny
    .from('mfa_unlock_requests')
    .update({ status: 'denied', denied_by: user.id, denied_at: now })
    .eq('id', id)
    .select()
    .single();
