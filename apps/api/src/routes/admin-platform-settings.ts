...
import { updatePlatformSettingsSchema } from '@brighttale/shared/schemas/pipeline-settings'
...
export async function adminPlatformSettingsRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (req, reply) => {
    ...
    const { data, error } = await sb
      ...
      .select('*')
      .maybeSingle()
    if (error) throw new ApiError(500, error.message, 'PLATFORM_SETTINGS_FETCH_ERROR')
  ...
  app.patch('/', async (req, reply) => {
    ...
    if (error) throw new ApiError(500, error.message, 'PLATFORM_SETTINGS_UPDATE_ERROR')
    if (!data) throw new ApiError(404, 'Platform settings not found', 'PLATFORM_SETTINGS_NOT_FOUND')
