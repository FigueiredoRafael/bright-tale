/**
 * ReviewEngine — issue #242 Module 4: progress modal driven by useActiveStageRun.
 *
 * 4 scenarios:
...
vi.mock('@/hooks/use-manual-mode', () => ({ useManualMode: () => ({ enabled: false }) }));

const DRAFT_ID = 'draft-review-1';
...
function makeFetch() {
  return vi.fn().mockImplementation(async (url: string) => {
    ...
    if (u.includes(`/api/content-drafts/${DRAFT_ID}`) && !u.includes('/review') && !u.includes('/produce') && !u.includes('/events')) {
      return { ok: true, json: async () => ({ data: makeReviewDraftRow({}), error: null }) };
    }
...
function wrap() {
  ...
  return (
    <StandaloneProjectContextProvider
      ...
      initialStageResults={{ draft: { draftId: DRAFT_ID, draftTitle: 'Test', draftContent: '', completedAt: '' } }}
...
describe('ReviewEngine — issue #242: progress modal driven by stage_run status', () => {
  ...
  it('unmounts GenerationProgressFloat when stage_run reaches completed status', async () => {
    ...
    act(() => {
      setActiveRun({ runId: 'sr-review-1', status: 'completed', isActive: false, isFresh: false });
    });
    rerender(wrap());
  ...
  it('does not mount GenerationProgressFloat when no active run exists', async () => {
    ...
    render(wrap());
    await waitFor(() => expect(screen.queryByTestId('review-engine-root')).not.toBeNull());
