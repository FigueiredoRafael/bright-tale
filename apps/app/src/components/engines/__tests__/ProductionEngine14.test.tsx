/**
 * Slice 14.2 — ProductionEngine migration tests (TDD red→green→refactor).
 *
 * Slices covered:
...
const PROJECT_ID = 'proj-production-test';
const CHANNEL_ID = 'ch-prod';
...
describe('ProductionEngine — slice 1: renders under ProjectContextProvider (no actor)', () => {
  it('renders without throwing when wrapped in ProjectContextProvider', async () => {
    ...
    render(
      <ProjectContextProvider projectId={PROJECT_ID}>
        <ProductionEngine projectId={PROJECT_ID} trackId="t1" medium="blog" />
...
describe('ProductionEngine — slice 2: reads from ProjectContextProvider', () => {
  ...
  it('uses projectId from prop when wrapped in ProjectContextProvider', async () => {
    global.fetch = makeContextFetch();
    render(
      <ProjectContextProvider projectId={PROJECT_ID}>
        <ProductionEngine projectId={PROJECT_ID} trackId="t1" medium="video" />
      </ProjectContextProvider>,
    );
    await waitFor(() =>
      expect(screen.queryByTestId('ctx-error')).not.toBeInTheDocument(),
