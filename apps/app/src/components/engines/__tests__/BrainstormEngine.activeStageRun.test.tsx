/**
 * BrainstormEngine — issue #242 Module 5: progress modal driven by useActiveStageRun.
 *
 * 4 scenarios:
...
const SESSION_ID = 'bs-test-session-1';
...
function makeFetch() {
  return vi.fn().mockImplementation(async (url: string) => {
    ...
    // Reconnect check — return no running session
    if (u.includes('/api/brainstorm/sessions/running')) {
      return { ok: true, json: async () => ({ data: { session: null }, error: null }) };
    }
    return { ok: true, json: async () => ({ data: null, error: null }) };
  });
}
...
function wrap(sessionId?: string) {
  return (
    ...
    >
      <BrainstormEngine
        mode="generate"
        // Pass initialSession so sessionId state is set — required for sseUrl
        initialSession={sessionId ? { id: sessionId } : undefined}
        initialIdeas={[]}
      />
    </StandaloneProjectContextProvider>
  );
...
describe('BrainstormEngine — issue #242: progress modal driven by stage_run status', () => {
  ...
  it('unmounts GenerationProgressFloat when stage_run reaches completed status', async () => {
    ...
    act(() => {
      setActiveRun({ runId: 'sr-brainstorm-1', status: 'completed', isActive: false, isFresh: false });
    });
