/**
 * Slice 14.2 — PreviewEngine migration tests (TDD red→green→refactor).
 *
 * Slices covered:
...
const PROJECT_ID = 'proj-preview14-test';
const CHANNEL_ID = 'ch-preview14';

const PROJECT_ROW = {
  id: PROJECT_ID,
  channel_id: CHANNEL_ID,
  title: 'Preview14 Test Project',
  mode: 'step-by-step',
...
const STUB_DRAFT = {
  id: 'draft-prev14',
  title: 'Preview14 Draft',
  draft_json: { blog: { full_draft: '## Section\n\nBody.' } },
...
function makeContextFetch(opts: { channelId?: string | null; stagesResponse?: object } = {}) {
  ...
  return vi.fn().mockImplementation((url: string) => {
    ...
    if ((url as string).includes('/content-drafts/')) {
      return Promise.resolve({ ok: true, json: () => Promise.resolve({ data: STUB_DRAFT, error: null }) });
    }
...
describe('PreviewEngine — slice 1: renders under ProjectContextProvider (no actor)', () => {
...
describe('PreviewEngine — slice 2: reads from ProjectContextProvider', () => {
