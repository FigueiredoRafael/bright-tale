...
export const pipelineMachine = setup({
  ...
  on: {
    ...
    NAVIGATE: [
      { guard: ({ event }: any) => event.toStage === 'brainstorm', target: '.brainstorm' },
      { guard: ({ event }: any) => event.toStage === 'research', target: '.research' },
      { guard: ({ event }: any) => event.toStage === 'draft', target: '.draft' },
      { guard: ({ event }: any) => event.toStage === 'review', target: '.review' },
      { guard: ({ event }: any) => event.toStage === 'assets', target: '.assets' },
      { guard: ({ event }: any) => event.toStage === 'preview', target: '.preview' },
      { guard: ({ event }: any) => event.toStage === 'publish', target: '.publish' },
    ],
    REDO_FROM: [
      {
        guard: ({ event }: any) => event.fromStage === 'brainstorm',
        target: '.brainstorm',
        actions: 'clearStrictlyAfter',
      },
      {
        guard: ({ event }: any) => event.fromStage === 'research',
        target: '.research',
        actions: 'clearStrictlyAfter',
      },
      {
        guard: ({ event }: any) => event.fromStage === 'draft',
        target: '.draft',
        actions: 'clearStrictlyAfter',
      },
      {
        guard: ({ event }: any) => event.fromStage === 'review',
        target: '.review',
        actions: 'clearStrictlyAfter',
      },
      {
        guard: ({ event }: any) => event.fromStage === 'assets',
        target: '.assets',
        actions: 'clearStrictlyAfter',
      },
      {
        guard: ({ event }: any) => event.fromStage === 'preview',
        target: '.preview',
        actions: 'clearStrictlyAfter',
      },
    ],
