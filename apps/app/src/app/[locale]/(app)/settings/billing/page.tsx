...
interface Status {
    ...
    // Mirrors CreditBalance from apps/api/src/lib/credits/balance.ts.
    credits: {
        unlimited: boolean;
        creditsTotal: number;
        creditsUsed: number;
...
export default function BillingPage() {
    ...
    const usagePct = useMemo(() => {
        if (!status || status.credits.unlimited) return 0;
        const total = (status.credits.creditsTotal ?? 0) + (status.credits.creditsAddon ?? 0);
        if (total === 0) return 0;
        return Math.min(100, ((status.credits.creditsUsed ?? 0) / total) * 100);
    }, [status]);
                    ...
                    <CardContent className="py-5">
                        ...
                        <div className="mt-5">
                            {status.credits.unlimited ? (
                                <div className="text-sm font-medium">Créditos ilimitados (VIP)</div>
                            ) : (
                                <>
                                    <div className="flex items-center justify-between text-xs mb-1.5">
                                        <span className="text-muted-foreground">Créditos usados</span>
                                        <span className="tabular-nums">
                                            {(status.credits.creditsUsed ?? 0).toLocaleString("pt-BR")} / {((status.credits.creditsTotal ?? 0) + (status.credits.creditsAddon ?? 0)).toLocaleString("pt-BR")}
                                        </span>
                                    </div>
