...
│   └── seed.sql                ← generated by scripts/seed-agents.ts (source: scripts/agents/*.ts)
└── scripts/
    └── agents/                 ← agent prompt definitions (.ts) — source of truth for agent_prompts
```
...
# Seed agent prompts
npm run db:seed       # runs db:seed:agents + db:seed:personas
npm run db:seed:agents # tsx scripts/seed-agents.ts → regenerates supabase/seed.sql + refresh migration from scripts/agents/*.ts
```
...
- Agent prompt definitions (source of truth): `scripts/agents/*.ts` (brainstorm, research, content-core, blog/video/shorts/podcast/engagement, review, assets, personas) → seeded into `agent_prompts` via `npm run db:seed:agents`. Instructions are assembled by `assembleInstructions()` in `@brighttale/shared`; persona/channel context is injected at runtime by the prompt builders in `apps/api/src/lib/ai/prompts/*`, not baked into the seed.
