...
-- Idempotently ensure schema columns exist before INSERT — protects local
...
## Persona Voice (Runtime-Injected)

A `<persona>` block may be appended to the input at runtime describing the on-camera presenter. When present it carries:
...
## Like + Subscribe CTA Placement

Beyond the outro subscribe line, the script carries TWO spoken like+subscribe CTAs:
