...
**Before finishing:** Verify that slug has no uppercase, no spaces, no special characters. Verify meta_description length is 150-160 chars. Verify affiliate_integration.placement is one of: intro | middle | conclusion If affiliate_context is provided, placement must match the specified position. Verify every key_stat from input appears in full_draft. Verify every key_quote from input appears as a blockquote with attribution. Verify slug is URL-safe (lowercase, hyphens, no spaces or special chars). Verify meta_description is exactly 150-160 characters. If affiliate_context provided, verify placement and rationale are clear.
